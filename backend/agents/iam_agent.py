"""IAMAgent — Identity and access management audit sub-agent (Section 5.2).

Tools: jwt_tool, oauthscan, saml_raider, modlishka, o365spray
"""
...
IAM_SYSTEM_PROMPT = """You are an IAM security audit agent. Test authentication and authorization.

Available tools: jwt_tool, oauthscan, saml_raider, modlishka, o365spray

Respond with JSON: {"tool": "name", "input": {"target": "...", "flags": "..."}, "reasoning": "...", "is_terminal": false}
When done: {"tool": null, "input": {}, "reasoning": "IAM audit complete", "is_terminal": true}"""


@dataclass
class IAMAgent(BaseAgent):
    """IAM security audit agent."""

    agent_type: AgentType = AgentType.IAM
    engine: EngineType = EngineType.INFRASTRUCTURE
    allowed_tools: frozenset[str] = field(
        default_factory=lambda: frozenset({"jwt_tool", "oauthscan", "saml_raider", "modlishka", "o365spray"})
    )
    ...
    async def _plan_next_action(self, task: AgentTask) -> AgentAction | None:
        ...
        if self.llm_router:
            return await _plan_with_llm(self, task, target, IAM_SYSTEM_PROMPT)
        return self._plan_fallback(target)

    def _plan_fallback(self, target: str) -> AgentAction | None:
        step = len(self._action_history)
        steps = [
            AgentAction("jwt_tool", {"target": target}, "JWT token analysis"),
            AgentAction("oauthscan", {"target": target}, "OAuth2 flow vulnerability scan"),
            AgentAction("saml_raider", {"target": target}, "SAML security testing"),
        ]
        if step >= len(steps):
            return None
        action = steps[step]
