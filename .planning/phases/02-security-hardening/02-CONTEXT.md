...
- **D-10 (resolved after research):** Use Docker-outside-of-Docker (DooD) — mount `/var/run/docker.sock` into the backend container and call the `docker` SDK for Python (docker-py 7.2.0, verified against PyPI) directly from `sandbox.py`. This was an explicit open question from research: `OPTIMUS_PRIME_ARCHITECTURE.md` §15.5 designed sandbox lifecycle around a host-level sidecar process (mirroring `backend/tools/sandbox_manager.py`'s existing `RuntimeWatchdog`) specifically to avoid mounting the Docker socket into a container. DooD was chosen anyway as the smaller change — the backend container gains host-level Docker access as a result. Acceptable given: (a) this is a personal-use, single-operator tool per CLAUDE.md, (b) the sandbox backend has zero live callers this phase (D-02), so the exposure isn't exercised yet. Requires a `docker-compose.yml` change to mount the socket and add `docker` to `backend/requirements.txt`.
