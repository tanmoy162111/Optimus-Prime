...
## From Plan 03-01 (full-suite verification, Task 1-3 completion check)
...
- **`tests/api/test_auth.py::test_chat_without_token_returns_401`** — FAILING (pre-existing).
  ...
  when a bearer token is present but invalid. This is unrelated to `backend/config.py`
  or `backend/memory/smart_memory.py` (this plan's only modified files). Not touched
  by Plan 03-01 Tasks 1-3. Candidate fix (future plan): either update the test's
